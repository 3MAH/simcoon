...
Col<int> array2Col_int(bn::ndarray const &array, const bool &copy) {
    ...
    bool strides_ok = copy;
    int n_rows = shape[0];
    if ((strides[0] != sizeof(int)) && (copy == false)) {
        cout << "error, the stride does not correspond to an array of int";
        exit(0);
    }
        
    Col<int> v(reinterpret_cast<int*>(array.get_data()), n_rows, copy);
    return v;
}

bn::ndarray Col_int2array(const Col<int> &v, const bool &copy) {
...
Mat<int> array2Mat_int(bn::ndarray const &array, const bool &copy) {
    ...
    if (copy == true) {
        if(is_C_contiguous) {
            Mat<int> m(reinterpret_cast<int*>(array.get_data()), n_cols, n_rows, false);
            return m.t();
        }
        else if (is_F_contiguous) {
            Mat<int> m(reinterpret_cast<int*>(array.get_data()), n_rows, n_cols, true);
            return m;
        }
        else {
            PyErr_SetString(PyExc_TypeError, "Array must be contiguous (C or F)");
            bp::throw_error_already_set();
            return zeros<Mat<int> >(1,1);
        }
...
bn::ndarray Mat_int2array(const Mat<int> &m, const bool &copy, const std::string& contiguous) {
    ...
    if (copy == true) {
        ...
        //create a tuple with the size of m
        bp::tuple shape = bp::make_tuple(m_t.n_cols, m_t.n_rows);
        bp::tuple stride = bp::make_tuple(sizeof(int) * m_t.n_rows, sizeof(int));
        bn::ndarray py_array = bn::from_data(m_t.memptr(),dtype,shape,stride,own);
        if (contiguous == "C") {
            return py_array.copy();
        }
        ...
        if (contiguous == "C") {
            bp::tuple shape = bp::make_tuple(m.n_cols, m.n_rows);
            bp::tuple stride = bp::make_tuple(sizeof(int) * m.n_rows, sizeof(int));
            bn::ndarray py_array = bn::from_data(m.memptr(),dtype,shape,stride,own);
            return py_array;
        }
        else if (contiguous == "F") {
            ...
            bp::throw_error_already_set();
            bp::tuple shape = bp::make_tuple(1, 1);
            bp::tuple stride = bp::make_tuple(sizeof(int), sizeof(int));
            bn::ndarray py_array = bn::from_data(m.memptr(),dtype,shape,stride,own);
            return py_array;
        }
    }
