...
///@file TELISO.cpp
///@brief Test for isotropic user material
///@version 1.0
...
TEST(TELISO, ELISO_solver )
{
    ...
    ASSERT_NO_THROW(solver_essentials(solver_type, corate_type, path_data, sol_essentials)) << "Failed to read solver essentials";
    ASSERT_NO_THROW(solver_control(div_tnew_dt_solver, mul_tnew_dt_solver, miniter_solver, maxiter_solver, inforce_solver, precision_solver, lambda_solver, path_data, sol_control)) << "Failed to read solver control";
    
    ASSERT_NO_THROW(read_matprops(umat_name, nprops, props, nstatev, psi_rve, theta_rve, phi_rve, path_data, materialfile)) << "Failed to read material properties";
