...
class Data:
    ...
    def __init__(self, control: npt.NDArray[np.float_],
        ...
        self.increments: npt.NDArray[np.float_] = np.asarray([i + 1 for i in range(np.shape(self.control)[0])])
        if time is None:
            self.time = np.asarray([i * timestep for i in range(np.shape(self.control)[0])])
        else:
            self.time = time


def write_input_and_tab_files(list_data: List[Data], exp_data_path: str = "exp_data/",
                              tab_files_path: str = "data/") -> None:
    for element in list_data:
        i = 1
        exp_data_array = np.column_stack((element.increments, element.time, element.control, element.observation))
        tab_file_array = np.column_stack((element.increments, element.time, element.control))
        np.savetxt(exp_data_path + "input_data_" + f'{i: 02}' + ".txt", exp_data_array)
        np.savetxt(tab_files_path + "tab_file_" + f'{i: 02}' + ".txt", tab_file_array)
        i += 1


def write_files_exp(list_data: List[Data], path: str = "data/", exp_data_path: str = "exp_data/") -> None:
    ...
    for element in list_data:
        nb_columns_in_files = element.increments.ndim + element.time.ndim + element.control.ndim + \
                              element.observation.ndim
        nb_observation_columns = element.observation.ndim
        observation_columns_indices = [i for i in range(nb_columns_in_files - element.observation.ndim, nb_columns_in_files)]
