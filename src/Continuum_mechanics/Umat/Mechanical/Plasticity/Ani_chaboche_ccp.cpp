...
void umat_ani_chaboche_CCP(const string &umat_name, const vec &Etot, const vec &DEtot, vec &stress, mat &Lt, mat &L, const mat &DR, const int &nprops, const vec &props, const int &nstatev, vec &statev, const double &T, const double &DT, const double &Time, const double &DTime, double &Wm, double &Wm_r, double &Wm_ir, double &Wm_d, const int &ndi, const int &nshr, const bool &start, double &tnew_dt, const int &tangent_mode)
{
    ...
    vec Lambdap = dAni_stress(stress-X,Ani_params);
    vec Lambdaa_1 = dAni_stress(stress-X,Ani_params) - D_1*a_1;
    vec Lambdaa_2 = dAni_stress(stress-X,Ani_params) - D_2*a_2;
    ...
    if (use_cpp) {
        ...
        auto solve_state = [&](const vec &sig, double Dl) {
            for (int it_fp = 0; it_fp < 20; it_fp++) {
                const vec eta_xi = dAni_stress(sig - X, Ani_params);
        ...
        auto probe_Phi = [&](const vec &sig, double Dl) {
            ...
            solve_state(sig, Dl);
            const double v = Ani_stress(sig - X, Ani_params) - Hp - sigmaY;
        ...
        mech.Phi            = [&](const vec &sig) { return Ani_stress(sig - X, Ani_params) - Hp - sigmaY; };
        ...
        mech.Lambda         = [&](const vec &sig) { return dAni_stress(sig - X, Ani_params); };
        mech.dLambda_dsigma = [&](const vec &sig) {
            ...
            for (int c6 = 0; c6 < 6; c6++) {
                ...
                sm(c6) -= hfd;
                solve_state(sp, Dl);
                const vec ep = dAni_stress(sp - X, Ani_params);
                solve_state(sm, Dl);
                const vec em = dAni_stress(sm - X, Ani_params);
        ...
        rm = closest_point_return_mapping(stress, L, mech,
                 ...
                 [&](const vec &sig, double Dl) -> vec {
                     ...
                     const double Dlm = std::max(Dl - hDl, 0.);
                     solve_state(sig, Dlp);
                     const vec ep = dAni_stress(sig - X, Ani_params);
                     solve_state(sig, Dlm);
                     const vec em = dAni_stress(sig - X, Ani_params);
                     X_1 = Xs1; X_2 = Xs2; X = Xs; a_1 = as1; a_2 = as2;
                     Hp = Hs; dHpdp = dHs;
        ...
        if (rm.converged) {
            ...
            Lambdap = dAni_stress(stress - X, Ani_params);       // flow direction (NOT the total dPhi/dsigma)
            ...
            p = p_n;
            Ds_j(0) = 0.;
            dPhidsigma = dAni_stress(stress - X, Ani_params);
            Lambdap = dPhidsigma;
    ...
    for (compteur = 0; ((compteur < simcoon::maxiter_umat) && (error > simcoon::precision_umat)); compteur++) {
        ...
        dPhidp = -1.*dHpdp;
//        dPhidp = -1.*dHpdp;
        dPhida_1 = -1.*(2./3.)*C_1*(dAni_stress(stress-X,Ani_params)%Ir05());
        dPhida_2 = -1.*(2./3.)*C_2*(dAni_stress(stress-X,Ani_params)%Ir05());
//        dPhida = 0.*(eta_stress(stress - X)%Ir05());
        
        //compute Phi and the derivatives
        Phi(0) = Ani_stress(stress-X,Ani_params) - Hp - sigmaY;
        
        Lambdap = dAni_stress(stress-X,Ani_params);
        Lambdaa_1 = dAni_stress(stress-X,Ani_params) - D_1*a_1;
        Lambdaa_2 = dAni_stress(stress-X,Ani_params) - D_2*a_2;
        kappa_j[0] = L*Lambdap;
    ...
    if (use_cpp) {
        ...
        // Simo-Hughes algorithmic tangent (closest-point). Ani flow on effective stress (sigma-X):
        // dLambda_eps/dsigma = ddAni_stress(stress-X). Backstress state-coupling deferred (CPP, future).
        const std::vector<mat> dLambda_dsigma_l = { ddAni_stress(stress - X, Ani_params) };
