...
void umat_dfa_chaboche_CCP(const string &umat_name, const vec &Etot, const vec &DEtot, vec &stress, mat &Lt, mat &L, const mat &DR, const int &nprops, const vec &props, const int &nstatev, vec &statev, const double &T, const double &DT, const double &Time, const double &DTime, double &Wm, double &Wm_r, double &Wm_ir, double &Wm_d, const int &ndi, const int &nshr, const bool &start, double &tnew_dt, const int &tangent_mode)
{
    ...
    vec Lambdap = dDFA_stress(stress-X,DFA_params);
    vec Lambdaa_1 = dDFA_stress(stress-X,DFA_params) - D_1*a_1;
    vec Lambdaa_2 = dDFA_stress(stress-X,DFA_params) - D_2*a_2;
    ...
    if (use_cpp) {
        ...
        auto solve_state = [&](const vec &sig, double Dl) {
            for (int it_fp = 0; it_fp < 20; it_fp++) {
                const vec eta_xi = dDFA_stress(sig - X, DFA_params);
        ...
        auto probe_Phi = [&](const vec &sig, double Dl) {
            ...
            solve_state(sig, Dl);
            const double v = DFA_stress(sig - X, DFA_params) - Hp - sigmaY;
        ...
        mech.Phi            = [&](const vec &sig) { return DFA_stress(sig - X, DFA_params) - Hp - sigmaY; };
        ...
        mech.Lambda         = [&](const vec &sig) { return dDFA_stress(sig - X, DFA_params); };
        mech.dLambda_dsigma = [&](const vec &sig) {
            ...
            for (int c6 = 0; c6 < 6; c6++) {
                ...
                sm(c6) -= hfd;
                solve_state(sp, Dl);
                const vec ep = dDFA_stress(sp - X, DFA_params);
                solve_state(sm, Dl);
                const vec em = dDFA_stress(sm - X, DFA_params);
        ...
        rm = closest_point_return_mapping(stress, L, mech,
                 ...
                 [&](const vec &sig, double Dl) -> vec {
                     ...
                     const double Dlm = std::max(Dl - hDl, 0.);
                     solve_state(sig, Dlp);
                     const vec ep = dDFA_stress(sig - X, DFA_params);
                     solve_state(sig, Dlm);
                     const vec em = dDFA_stress(sig - X, DFA_params);
                     X_1 = Xs1; X_2 = Xs2; X = Xs; a_1 = as1; a_2 = as2;
                     Hp = Hs; dHpdp = dHs;
        ...
        if (rm.converged) {
            ...
            Lambdap = dDFA_stress(stress - X, DFA_params);       // flow direction (NOT the total dPhi/dsigma)
            ...
            p = p_n;
            Ds_j(0) = 0.;
            dPhidsigma = dDFA_stress(stress - X, DFA_params);
            Lambdap = dPhidsigma;
    ...
    for (compteur = 0; ((compteur < simcoon::maxiter_umat) && (error > simcoon::precision_umat)); compteur++) {
        ...
        dPhidp = -1.*dHpdp;
//        dPhidp = -1.*dHpdp;
        dPhida_1 = -1.*(2./3.)*C_1*(dDFA_stress(stress-X,DFA_params)%Ir05());
        dPhida_2 = -1.*(2./3.)*C_2*(dDFA_stress(stress-X,DFA_params)%Ir05());
//        dPhida = 0.*(eta_stress(stress - X)%Ir05());
        
        //compute Phi and the derivatives
        Phi(0) = DFA_stress(stress-X,DFA_params) - Hp - sigmaY;
        
        Lambdap = dDFA_stress(stress-X,DFA_params);
        Lambdaa_1 = dDFA_stress(stress-X,DFA_params) - D_1*a_1;
        Lambdaa_2 = dDFA_stress(stress-X,DFA_params) - D_2*a_2;
        kappa_j[0] = L*Lambdap;
    ...
    if (use_cpp) {
        ...
        // Simo-Hughes algorithmic tangent (closest-point). DFA flow on effective stress (sigma-X):
        // dLambda_eps/dsigma = ddDFA_stress(stress-X). Backstress state-coupling deferred (CPP, future).
        const std::vector<mat> dLambda_dsigma_l = { ddDFA_stress(stress - X, DFA_params) };
