...
void umat_plasticity_hill_isoh_CCP(const string &umat_name, const vec &Etot, const vec &DEtot, vec &stress, mat &Lt, mat &L, const mat &DR, const int &nprops, const vec &props, const int &nstatev, vec &statev, const double &T, const double &DT, const double &Time, const double &DTime, double &Wm, double &Wm_r, double &Wm_ir, double &Wm_d, const int &ndi, const int &nshr, const bool &start, double &tnew_dt, const int &tangent_mode)
{
    ...
    // For the ANISOTROPIC Hill flow this is where the integrator difference matters: the CPP map
    // is symmetric and its consistent tangent exact. Condensed states (ndi < 3) keep the CCP loop.
    ...
    if (use_cpp) {
        ...
        mech.Phi            = [&](const vec &sig) { return Hill_stress(sig, Hill_params) - Hp - sigmaY; };
        mech.dPhi_dsigma    = [&](const vec &sig) { return dHill_stress(sig, Hill_params); };
        mech.dLambda_dsigma = [&](const vec &sig) { return ddHill_stress(sig, Hill_params); };
    ...
    if (use_cpp) {
        ...
        // symmetric and machine-exact also for the ANISOTROPIC Hill flow.
