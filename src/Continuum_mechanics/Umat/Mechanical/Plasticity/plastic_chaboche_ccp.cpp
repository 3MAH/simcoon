    ...
    if (use_cpp) {
        ...
        auto solve_state = [&](const vec &sig, double Dl) {
            for (int it_fp = 0; it_fp < 20; it_fp++) {
                const vec eta_xi = eta_stress(sig - X);
        ...
        auto probe_Phi = [&](const vec &sig, double Dl) {
            ...
            solve_state(sig, Dl);
            const double v = Mises_stress(sig - X) - Hp - sigmaY;
        ...
        mech.dLambda_dsigma = [&](const vec &sig) {
            ...
            for (int c6 = 0; c6 < 6; c6++) {
                ...
                sm(c6) -= hfd;
                solve_state(sp, Dl);
                const vec ep = eta_stress(sp - X);
                solve_state(sm, Dl);
                const vec em = eta_stress(sm - X);
        ...
        rm = closest_point_return_mapping(stress, L, mech,
                 ...
                 [&](const vec &sig, double Dl) -> vec {
                     ...
                     const double Dlm = std::max(Dl - hDl, 0.);
                     solve_state(sig, Dlp);
                     const vec ep = eta_stress(sig - X);
                     solve_state(sig, Dlm);
                     const vec em = eta_stress(sig - X);
                     X_1 = Xs1; X_2 = Xs2; X = Xs; a_1 = as1; a_2 = as2;
                     Hp = Hs; dHpdp = dHs;
